...
class SpeciesTdoaParams(BaseModel):
    ...
    correlation_method: str = Field(
        ...
        description="'plain' (time-domain cross-correlation, weighted by "
                     "actual signal energy), 'gcc_phat' (per-bin phase "
                     "normalization applied to already-bandpass-filtered "
                     "audio — no species currently uses this; "
                     "docs/tdoa-correlation-design-notes.md section 7 found "
                     "it worse than 'plain' once bandpass filtering is "
                     "already applied upstream, since whitening re-amplifies "
                     "the filter's residual stopband content), or "
                     "'phat_masked' (per-bin phase normalization of the RAW "
                     "cross-spectrum, with the species' frequency band "
                     "applied AFTER whitening instead of before — fixes "
                     "'gcc_phat''s filter-then-whiten cancellation bug; the "
                     "production default for all species as of 2026-07-31, "
                     "see project_soundhub_phat_masked_calibrated_thresholds "
                     "memory for the trust-gate calibration behind it). An "
