"""
calibrate_phat_masked_coef.py — derives real MIN_PEAK_CORR_COEF/
AMBIGUOUS_RATIO_THRESHOLD candidates for phat_masked, using the same
synthetic ground-truth harness as validate_toa_methods.py (real Xeno-canto
calls, known injected delay).

WHY THIS EXISTS (2026-07-31): production's existing trust-gate thresholds
(MIN_PEAK_CORR_COEF=0.3, AMBIGUOUS_RATIO_THRESHOLD=1.2 in server/
correlation.py) were tuned around 'plain' amplitude correlation's
coefficient scale. Confirmed on real DB-backed attempts
(validate_toa_real_pulls_v2.py, real per-attempt species bands):
phat_masked's properly-normalized coefficient sat around 0.05-0.09 even on
plausible-looking real matches — nowhere near 0.3 — because a whitened
coefficient behaves differently from a raw-amplitude one even given an
identical band/window. Reusing plain's thresholds for phat_masked would
reject almost everything regardless of match quality.

WHY THE SYNTHETIC HARNESS, NOT REAL DATA: unlike real field pulls, every
trial here has a KNOWN true injected delay, so "correct" vs "incorrect"
isn't a judgement call — a trial is "correct" if its estimated lag lands
within --tolerance-us of the known true lag, "incorrect" otherwise. That
lets peak_corr_coef/quality_ratio be split into two clean, independently
labelled populations and examined directly, rather than guessing at a
threshold from real data where correctness is itself unknown (see
project_soundhub_residual_self_consistency_degenerate/project_soundhub_
solver_inward_bias_confirmed for why real-data *position* correctness
currently can't be used as that label at this aperture — lag-level
labelling from synthetic ground truth sidesteps that problem entirely).

'raw' (plain amplitude correlation) is included as a sanity-check
baseline, not a calibration target — it's already tuned. If raw's
EXISTING 0.3/1.2 thresholds don't show clean separation between correct
and incorrect trials in this same harness, that would mean the harness/
methodology itself isn't trustworthy, and phat_masked's numbers shouldn't
be trusted either until that's understood.

Reuses validate_toa_methods.py's species config loading, call-finding, and
noise/gain trial construction, plus its _corr_phat_masked/_score_from_corr
helpers and production's own 'plain' _score_correlation.

Report-only: does not modify server/correlation.py or
config/species_tdoa_params.json.

Usage:
    python tools/calibrate_phat_masked_coef.py
    python tools/calibrate_phat_masked_coef.py --n-trials 30 --json-out calib_report.json

Requires: numpy, scipy, ffmpeg on PATH. Run from the sound-hub root directory.
"""
...
# Production's current thresholds (server/correlation.py) -- tuned for
# 'plain', included here purely as the sanity-check reference point.
PROD_MIN_PEAK_CORR_COEF = prod_correlation.MIN_PEAK_CORR_COEF
PROD_AMBIGUOUS_RATIO_THRESHOLD = prod_correlation.AMBIGUOUS_RATIO_THRESHOLD

CANDIDATE_PERCENTILES = [5, 10, 20]


def _run_one_trial(raw_data, rate, onset_idx, pre_samples, post_samples,
                    transit_samples, true_lag_samples, snr_db, gain, band_lo, band_hi, rng):
    """Same trial construction as validate_toa_methods._run_one_trial, but
    only scores raw ('plain') and phat_masked, and returns full score dicts
    (lag_us, peak_corr_coef, quality_ratio) instead of just lag_us -- this
    script needs the coefficient/ratio values themselves, not just the
    error, to build the calibration distributions below."""
...
def _percentile_report(label: str, correct_vals: np.ndarray, incorrect_vals: np.ndarray):
    if len(correct_vals) == 0:
        print(f"    {label}: no correct trials -- cannot calibrate")
        return None
    pcts = [5, 10, 25, 50, 75, 90]
    c_pcts = np.percentile(correct_vals, pcts)
    print(f"    {label} | correct   (n={len(correct_vals):6d}): " +
          "  ".join(f"p{p}={v:6.3f}" for p, v in zip(pcts, c_pcts)))
    if len(incorrect_vals) > 0:
        i_pcts = np.percentile(incorrect_vals, pcts)
        print(f"    {label} | incorrect (n={len(incorrect_vals):6d}): " +
              "  ".join(f"p{p}={v:6.3f}" for p, v in zip(pcts, i_pcts)))
    else:
        print(f"    {label} | incorrect (n=0): no data")

    print(f"    {label} | candidate thresholds (keep this %ile of correct trials, "
          f"check false-accept rate on incorrect trials):")
    candidates = {}
    for p in CANDIDATE_PERCENTILES:
        thresh = float(np.percentile(correct_vals, p))
        false_accept = float(np.mean(incorrect_vals >= thresh)) if len(incorrect_vals) else float("nan")
        print(f"      keep {100 - p:3d}% of correct (threshold={thresh:7.3f}): "
              f"false-accept-rate-on-incorrect={false_accept*100:5.1f}%")
        candidates[f"p{p}"] = {"threshold": thresh, "false_accept_rate": false_accept}
    return candidates
...
def main():
    ...
    tmp_root = args.tmp_dir or tempfile.mkdtemp(prefix="calibrate_phat_masked_")

    # pooled[method]["coef"|"ratio"][True|False] -> list of values, keyed by whether that trial's OWN lag was correct
    ...
    try:
        for species in species_list:
            ...
            transit_samples = int(round(args.transit_ms * 1e-3 * next(iter(calls_by_file.values()))[0]))
            pad_samples = int(round(_EXTRA_PAD_MS * 1e-3 * next(iter(calls_by_file.values()))[0]))
            max_lag_samples = int(round(max(abs(v) for v in lags_ms) * 1e-3 * next(iter(calls_by_file.values()))[0]))
    ...
    print(f"\n=== Pooled across {len(species_list)} species "
          f"(tolerance={args.tolerance_us:.0f}us, {len(lags_ms)} lags x {len(snr_db_list)} SNR levels x {args.n_trials} trials) ===")
    ...
    print("\nSynthetic ground-truth calibration only -- does not modify server/correlation.py "
          "or config/species_tdoa_params.json. 'raw' is a sanity-check baseline (already tuned), "
